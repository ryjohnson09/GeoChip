...
ui <- fluidPage(
  ...
  titlePanel("Geochip Response Ratios"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output){
  ...
  geochip_final <- reactive({
    ...
    geochip_phylum() %>%
      ...
      gather(key = glomics_ID,
             value = Signal,
             -Genbank.ID, -Gene,
             -Organism, -Lineage, -Gene_category, -Subcategory1,
             -Subcategory2, -Phylum) %>%
  ...
  ### Merge geochip with TrEAT ###
  #################################
  geochip_treat <- reactive({
  ...
  plotInput <- reactive({
    ...
    group_labels <- geochip_treat() %>%
      select(!!grouping_cat) %>%
      distinct() %>%
      pull(!!grouping_cat)
    
    
    ggplot(data = geochip_significant()) +
  ...
  #output$table <- renderTable({head(geochip_significant(), 25)})
