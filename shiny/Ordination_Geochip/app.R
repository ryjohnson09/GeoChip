...
Gene_category_choices <- c("All", unique(na.omit(geochip$Gene_category)))
...
server <- function(input, output){
    ...
    if(input$matched == "matched_samples"){
      ...
      geochip %>%
        select_if(colnames(.) %in% c("Genbank.ID", "Gene", "Organism", 
                                     ...
                                     "Subcategory2", "Lineage", matched_glomics))
  ...
  ################################
  ### Filter by probe category ###
  ################################
  
  geochip_probe <- reactive({
    if(input$Gene_category != "All"){
      geochip_select() %>%
        filter(Gene_category == input$Gene_category)
    } else if (input$Gene_category == "All"){
      geochip_select() 
    } else {
      stopApp()
    }
  ...
  geochip_alpha <- reactive({
    
    # Inverse Simpson
    geochip_invs <- as.tibble(diversity(t(geochip_filtered()), "inv")) %>%
      rownames_to_column("glomics_ID") %>%
      rename(invsimpson = value)
    
    # Shannon
    geochip_shann <- as.tibble(diversity(t(geochip_filtered()), "shannon")) %>%
      rownames_to_column("glomics_ID") %>%
      rename(shannon = value)
    
    # Merge into One
    geochip_alpha_diversity <- geochip_invs %>%
      full_join(., geochip_shann, by = c("glomics_ID"))
    
    geochip_alpha_diversity
  })
  ...
  geochip_coords_treat <- reactive({
    
    geochip_coords() %>%
      left_join(., treat_pathogen_filtered(), by = c("glomics_ID")) %>%
      right_join(., geochip_alpha(), by = c("glomics_ID")) %>%
  ...
  output$plot<-renderPlot({
    ...
    # PCA Plot
    if(input$ordination == "PCA"){
    pca_plot <- ggplot(geochip_coords_treat(),
            aes_string(x = "PC1", 
                       y = "PC2", 
                       color = my_fill,
                       size = my_size)) +
    ...
    # DCA plot
    dca_plot <- ggplot(geochip_coords_treat(),
           aes_string(x = "DCA1",
                      y = "DCA2", 
                      color = my_fill,
                      size = my_size)) +
    ...
    # PCoA Plot
    pcoa_plot <- ggplot(geochip_coords_treat(), 
                aes_string(x = "Axis.1", 
                           y = "Axis.2", 
                           color = my_fill, 
                           size = my_size)) +
  ...
  #output$table <- renderTable({geochip_coords_treat()})
}
