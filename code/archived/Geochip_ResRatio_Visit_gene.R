...
# Factor for plot
geochip_RR_tidy$Gene <- factor(geochip_RR_tidy$Gene, 
                            levels = rev(sort(unique(geochip_RR_tidy$Gene))))
...
geochip_RR_plot <- ggplot(data = geochip_RR_tidy) +
  ...
  geom_point(aes(x = Gene, y = RR, color = RR_group), size = 3, position = dodge) +
...
ggsave("results/figures/Geochip_RespRatio_Visit_Gene.png", height = 18, width = 9)
