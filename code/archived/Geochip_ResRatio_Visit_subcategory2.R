...
geochip_RR <- geochip %>%
  ...
  select(-Gene_category, -Genbank.ID, -Subcategory1,  
         -Signal, -Organism, -Gene, -Lineage) %>%
...
genes_no_overlap_0 <- geochip_RR_tidy %>%
  mutate(overlap_zero = ifelse(0 > RR - CI95 & 0 < RR + CI95, TRUE, FALSE)) %>%
  filter(overlap_zero == FALSE) %>%
  distinct(Subcategory2) %>%
  pull(Subcategory2)

geochip_RR_tidy <- geochip_RR_tidy %>%
  filter(Subcategory2 %in% genes_no_overlap_0)

rm(genes_no_overlap_0)

# Factor for plot
geochip_RR_tidy$Subcategory2 <- factor(geochip_RR_tidy$Subcategory2, 
                            levels = rev(sort(unique(geochip_RR_tidy$Subcategory2))))
...
geochip_RR_plot <- ggplot(data = geochip_RR_tidy) +
  ...
  geom_point(aes(x = Subcategory2, y = RR, color = RR_group), size = 3, position = dodge) +
...
ggsave("results/figures/Geochip_RespRatio_Visit_Subcategory2.png", height = 5, width = 7)
