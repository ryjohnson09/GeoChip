#########################################################
# Name: Visit_unique_gene_venndiagram.R
# Author: Ryan Johnson
...
geochip1 <- geochip %>% 
  ...
  group_by(glomics_ID, Gene, study_id, visit_number, Gene_category, Subcategory1, Subcategory2) %>%
  summarise(gene_presense = sum(Signal)) %>%
  ungroup() %>%
  
  # Remove genes not detected in samples
...
# Get tibble of genes within each visit
visit_genes <- geochip2 %>%
  select(Gene, visit_number) %>%
  group_by(visit_number) %>%
  mutate(group_row = 1:n()) %>%
  spread(visit_number, Gene) %>%
  select(-group_row)


# Make three lists of unique genes at each visit.
Visit1 <- unique(na.omit(visit_genes$`1`))
Visit4 <- unique(na.omit(visit_genes$`4`))
Visit5 <- unique(na.omit(visit_genes$`5`))
...
visit_venn <- venn.diagram(visit_gene_list,
                           filename = "results/figures/Visit_unique_gene_venndiagram.tiff",
                           fill = c(2:4),
                           alpha = 0.3,
                           cex = 2,
                           cat.cex = 2)