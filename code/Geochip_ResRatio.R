...
## Load data --------------------------------------------------------------------------------------------
geochip <- suppressMessages(suppressWarnings(read_tsv("data/processed/Merged_Geochip.tsv")))
treat <- suppressMessages(suppressWarnings(read_csv("data/processed/TrEAT_Clinical_Metadata_tidy.csv")))
ID_Decoder <- suppressMessages(suppressWarnings(read_csv("data/processed/ID_Decoder_Geochip.csv")))
...
geochip_filtered <- geochip %>% 
  select_if(colnames(.) %in% c("Genbank.ID", "Gene", "Organism",
                               "Lineage", "Gene_category", "Subcategory1",
...
geo_RR <- geo_grouped %>% 
  ...
  group_by(Gene_category, visit_number, country) %>% 
...
ggsave(plot = RR_plot, filename = "results/figures/Geo_geneCat_country.png", height = 12, width = 7)
