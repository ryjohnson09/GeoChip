...
geochip_RR <- geochip %>%
  ...
  select(-Gene_category, -Genbank.ID, -Gene,  
...
geochip_RR_tidy <- geochip_RR_tidy %>%
  filter(Subcategory1 %in% genes_no_overlap_0)
...
ggsave("results/figures/Geochip_RespRatio_Visit_subcategory1.png", height = 7, width = 7)
