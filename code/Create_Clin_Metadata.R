...
# Filter for observations in ID_list that are found in the geochip_samples
ID_list_geochip <- ID_list %>% 
  filter(glomics_ID %in% colnames(geochip_samples))

# Extract unique patient ID's
geochip_patient_IDs <- unique(ID_list_geochip$study_id)

# Filter treat to include only those in geochip_patient_id
treat_geochip <- treat %>% 
  filter(STUDY_ID %in% geochip_patient_IDs)

# Clean
rm(geochip_patient_IDs)
...
treat_geochip_clin <- treat_geochip %>% 
  select(STUDY_ID, 
...
treat_geochip_clin <- treat_geochip_clin %>% 
