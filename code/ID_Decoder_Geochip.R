...
geochip_samples <- suppressMessages(suppressWarnings(read_tsv("data/processed/Merged_Geochip.tsv", n_max = 1))) %>%
  select(starts_with("X")) %>%
  colnames()

ID_list <- ID_list %>%
  filter(glomics_ID %in% geochip_samples)


## Write to processed data -----------------------
write_csv(ID_list, "data/processed/ID_Decoder_Geochip.csv")
