###########################################################
# Name: Geochip_ResRatio_Visit_gene.R
# Author: Ryan Johnson
...
geochip_RR <- geochip %>%
  ...
  select(-Genbank.ID, -Gene_category, -Subcategory2,  
...
geochip_RR_plot <- ggplot(data = arrange(geochip_RR, desc(RR_41))) +
  ...
  # 1 v 4
  geom_point(aes(x = RR_41, y = Gene), size = 4, color = "black") +
  geom_errorbarh(aes(xmin = RR_41 - CI95_41, xmax = RR_41 + CI95_41, y = Gene), 
                 color = "black") +
  # 1 v 5
  geom_point(aes(x = RR_51, y = Gene), size = 4, color = "red") +
  geom_errorbarh(aes(xmin = RR_51 - CI95_51, xmax = RR_51 + CI95_51, y = Gene), 
                 color = "red") +
  ggtitle("Response Ratio: Gene\nBlack = Visit 4 vs Visit 1\nRed = Visit 5 vs Visit 1") +
  ylab("Gene Category") +
  xlab("Response Ratio") +
  theme_minimal() +
  theme(
...
ggsave("results/figures/Geochip_RespRatio_Visit_Gene.png", height = 18, width = 7)
