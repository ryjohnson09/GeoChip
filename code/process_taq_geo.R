...
ID_Decoder <- read_csv("data/processed/ID_Decoder_Geochip.csv")
...
taq_data_visit1_format <- taq_data_visit1 %>%
  ...
  # filter for samples in geochip data
...
# Merge ont ID_Decoder so that only geochip samples are included
taq_data_geo <- ID_Decoder %>% 
...
taq_data_geo_either <- taq_data_geo %>% 
  # CMY
...
taq_data_geo_either <- taq_data_geo_either %>% 
...
write_csv(x = taq_data_geo_either, path = "data/processed/Taq_tidy.csv")
